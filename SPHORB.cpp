...
{
    // ref: https://blog.csdn.net/kingjo002/article/details/8442146
    if( access("Data/geoinfo64.pfm", F_OK) == -1
        || access("Masks/no_mask.png", F_OK) == -1)
    {
        cerr << "Please check directory ./Data and ./Masks ! One of them is missing or incomplete!" << endl;
        exit(1);
    }
...
// ref ```void ORB_Impl::detectAndCompute``` in orb.cpp
                        ...
                        OutputArray _descriptors, bool useProvidedKeypoints) const
{
    ...
    // add mask! 修改代码的紧跟技巧！ where is a "temp", there is a temp_mask!
    Mat temp_mask = _mask.getMat();
    // 如果 mask 用默认参数 Mat()，则使用 no_mask.png
    if(temp_mask.empty())
    {
        temp_mask = imread("./Masks/no_mask.png", IMREAD_GRAYSCALE);
    }

    Mat descriptors;
    if( temp.type() != CV_8UC1 )
        cvtColor(_image, temp, COLOR_BGR2GRAY);
    if( temp_mask.type() != CV_8UC1 )
        cvtColor(temp_mask, temp_mask, COLOR_BGR2GRAY);  // temp_mask紧跟temp
    ...
    for (int l=0;l<nlevels;l++)
    {
        ...
        //!!! NOTE! 5 is the split part! 1280/5 = 256, the size of the image in the pyramid bottom
        Size sz(cells[l]*5, cells[l]*5/2);
        Mat image(sz, temp.type());
        resize(temp, image, sz, 0, 0, INTER_AREA);
        // temp_mask紧跟temp，此时，mask相应的金字塔已经建立
        Mat mask(sz, temp_mask.type());
        resize(temp_mask, mask, sz, 0, 0, INTER_AREA);

        // split the spherical image to five parts
        Mat subImg[5];
        Mat subImg_mask[5];  // subImg_mask 紧跟 subImg
        for(int i=0;i<5;i++)
        {
            subImg[i].create(Size(2*cells[l]+1, cells[l]+1), image.type());
            splitSphere2(image, subImg[i], i, imgInfos[l][i]);
            // subImg_mask 紧跟 subImg
            subImg_mask[i].create(Size(2*cells[l]+1, cells[l]+1), mask.type());
            splitSphere2(mask, subImg_mask[i], i, imgInfos[l][i]);
        }

        // extend each part for boundary pixels
        extendEdge(subImg[0], subImg[1], subImg[2], subImg[3], subImg[4], SFAST_EDGE + SPHORB_EDGE);
        // subImg_mask 紧跟 subImg
        extendEdge(subImg_mask[0], subImg_mask[1], subImg_mask[2], subImg_mask[3], subImg_mask[4], SFAST_EDGE + SPHORB_EDGE);

        // the key points on each dd
...
//            // subImg_mask 紧跟 subImg
//            corners = sfast_corner_detect(
//                &subImg[i].at<uchar>(0,0), &subImg_mask[i].at<uchar>(0,0),&maskes[l].at<uchar>(0,0),
...
//            delete[] score;
            ...
            sfast(subImg[i], subImg_mask[i], maskes[l], barrier, true, i, partKeyPoints);
